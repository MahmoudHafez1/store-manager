...
class EditExporterFrom extends Component {
    ...
    onSubmitHandler(e){
        ...
        axios.patch("http://localhost:5000/exporters/edit/"+this.props.name,updatedData)
        .then(res=>{
            alert(res.data)
            if(res.data==="Updated Successfully"){
                axios.patch("http://localhost:5000/operations/exporter/"+this.props.name,updatedData)
                .then(res=>{
                 console.log(res.data)
                 window.location.href="http://localhost:3000/exporters/"+this.state.newName
                })
            }
        })
...
export default EditExporterFrom;